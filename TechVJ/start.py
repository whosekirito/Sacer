...
async def send_cancel(client: Client, message: Message):
    ...
    # Check if user has any ongoing batch process
    if user_id in batch_temp.IS_BATCH and not batch_temp.IS_BATCH[user_id]:
        batch_temp.IS_BATCH[user_id] = True
        await client.send_message(
            chat_id=message.chat.id, 
            text="**✅ Batch Process Successfully Cancelled!**\n\n🔄 You can now start new downloads."
        )
    else:
        await client.send_message(
            chat_id=message.chat.id, 
            text="**ℹ️ No active batch process found to cancel.**"
        )

# Force cancel batch command (alternative command)
@Client.on_message(filters.command(["cancelall", "stopbatch"]))
async def force_cancel_batch(client: Client, message: Message):
    user_id = message.from_user.id
    batch_temp.IS_BATCH[user_id] = True

    await client.send_message(
        chat_id=message.chat.id, 
        text="**⛔ All batch processes forcefully cancelled!**\n\n🔄 Ready for new downloads."
    )
